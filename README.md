...
A separate **InterBio/2026** workspace (`~/Documents/Internship/InterBio/2026`) syncs these logs:
it pulls `hermes-vps:~/magang/{data/,config.yaml}` over the `hermes-vps` alias and, when agents log
locally through its proxy, pushes changed `data/pekan-NN.yaml` files back up (rsync `--update`, no
`--delete`; this VPS is the source of truth; `config.yaml` is pull-only). So
